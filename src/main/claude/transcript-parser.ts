...
/**
 * Patterns Claude Code uses to wrap synthetic/meta content inside otherwise
 * `type: 'user'` lines. These are noise from the chat UX perspective —
 * caveats, slash-command echoes, command stdout snippets, confirmation
 * blurbs from settings commands — so we drop them entirely. The CLI view
 * still surfaces them for debugging.
 *
 * Patterns are matched anywhere in the text (not just prefix) because
 * Claude sometimes wraps the marker in extra whitespace or boilerplate.
 */
const META_PATTERNS = [
  /<command-name>/i,
  /<command-message>/i,
  ...
  /<bash-stdout>/i,
  /<bash-stderr>/i,
  // Claude prepends this caveat whenever it injects local-command output
  // into the conversation — never useful in a chat view.
  /Caveat:\s+The messages below were generated by the user while running local commands/i,
  // Echo of a slash command we (or the user via /-prefix) sent — looks
  // like `/effort effort high`, `/model opus`, `/clear`, etc. Anchored
  // to the very start of the trimmed line so we don't match real prose
  // that happens to contain a slash.
  /^\/[a-z][a-z0-9-]{0,30}(\s+[\w-]+){0,3}\s*$/i,
  // Settings confirmation blurbs from claude's slash commands.
  /^Set\s+\w+\s+(level|to|mode)\s+to\s+\w+/i
]
...
function parseLine(line: string, index: number): TranscriptMessage | null {
  ...
  // Drop internal meta lines and subagent sidechains — they don't belong
  // in the main chat view (a future "show meta" toggle could surface them).
  ...
  // Filter user-type lines that are slash-command artefacts (caveats,
  // <command-name> echoes, captured stdout). They're recorded as user
  // turns by the JSONL convention but shouldn't render as a user bubble.
  if (type === 'user') {
    const everyBlockIsSynthetic = blocks.every(
      (b) => b.kind === 'text' && isSyntheticUserText(b.text)
    )
    if (everyBlockIsSynthetic) return null
  }
