...
Scope {
    ...
    Process {
        id: activeScreenProc
        command: ["bash", "-c", "hyprctl activeworkspace -j | python3 -c \"import sys,json; d=json.load(sys.stdin); print(d['monitor'])\""]
        stdout: SplitParser {
            onRead: data => {
                var monitorName = data.trim()
                for (var i = 0; i < Quickshell.screens.length; i++) {
                    if (Quickshell.screens[i].name === monitorName) {
                        launcherScope.activeScreen = Quickshell.screens[i]
                        break
                    }
                }
            }
    ...
    Variants {
        ...
        PanelWindow {
            ...
            margins {
                top: 40
                bottom: 10
                left: (root.launcherVisible && launcherScope.activeScreen === modelData) ? 6 : -450
            }
            implicitWidth: 420
            color: "transparent"
            focusable: true
            WlrLayershell.keyboardFocus: (root.launcherVisible && launcherScope.activeScreen === modelData) ? WlrKeyboardFocus.OnDemand : WlrKeyboardFocus.None
            ...
            Connections {
                target: root
                function onLauncherVisibleChanged() {
                    if (root.launcherVisible && launcherScope.activeScreen === modelData) {
                        searchInput.text = ""
                        wallSearchInput.text = ""
                        root.selectedIndex = 0
                        root.wallSelectedIndex = 0
            ...
            Timer {
                ...
                repeat: false
                onTriggered: {
                    if (launcherScope.activeScreen === modelData) {
