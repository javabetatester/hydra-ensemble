 ...
 * renders a compact explanation of which agent the router chose, the
 * score, the reason, and the sorted candidate list. No card wrapper —
 * this sits inline inside TaskDrawer's sections.
 *
...
type ReasonKind = 'fallback' | 'delegation' | 'scored'
...
export default function RouteExplain({ taskId }: Props) {
  ...
  // Build + sort candidate rows desc by score. Chosen first on ties.
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={`flex items-start gap-1.5 text-[11px] ${reasonClass}`}>
        ...
        <span className="break-words">{route.reason}</span>
      </div>
